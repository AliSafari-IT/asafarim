...
BACKEND_TESTS_DIR="$TESTS_DIR/backend"
FRONTEND_TESTS_DIR="$TESTS_DIR/frontend"
BACKEND_TEST_RESULTS_FILE="$BACKEND_TESTS_DIR/backend_test_results_${TIMESTAMP}.trx"
FRONTEND_TEST_RESULTS_FILE="$FRONTEND_TESTS_DIR/frontend_test_results_${TIMESTAMP}.json"
UI_TEST_RESULTS_FILE="$FRONTEND_TESTS_DIR/ui_test_results_${TIMESTAMP}.json"
BLOG_TEST_RESULTS_FILE="$FRONTEND_TESTS_DIR/blog_test_results_${TIMESTAMP}.json"
BIBLIOGRAPHY_TEST_RESULTS_FILE="$FRONTEND_TESTS_DIR/bibliography_test_results_${TIMESTAMP}.json"
...
# Run backend tests and check results
run_backend_tests_and_verify() {
  log "Running backend tests before deployment..."
  
  # Create test results directory if it doesn't exist
  mkdir -p "$BACKEND_TESTS_DIR"
  
  # Run the tests
  dotnet test "$BASE_DIR/ASafariM.Test/ASafariM.Test.csproj" --logger "trx;LogFileName=$BACKEND_TEST_RESULTS_FILE"
  TEST_EXIT_CODE=$?
  
  if [ $TEST_EXIT_CODE -ne 0 ]; then
    log "❌ Backend tests failed with exit code $TEST_EXIT_CODE. Aborting deployment."
    return 1
  fi
  
  # Check for failed tests in the TRX file
  if [ -f "$BACKEND_TEST_RESULTS_FILE" ]; then
    FAILED_TESTS=$(grep -c 'outcome="Failed"' "$BACKEND_TEST_RESULTS_FILE")
    if [ "$FAILED_TESTS" -gt 0 ]; then
      log "❌ $FAILED_TESTS backend tests failed. Aborting deployment."
      return 1
    else
      log "✅ All backend tests passed. Proceeding with deployment."
      return 0
    fi
  else
...
# Run frontend tests and check results
run_frontend_tests_and_verify() {
  log "Running frontend tests before deployment..."
  ...
  # Run the tests using the yarn test command with JSON reporter
  yarn test --reporter=json --outputFile="$FRONTEND_TEST_RESULTS_FILE"
  TEST_EXIT_CODE=$?
  
  # Also run individual tests for each frontend project with JSON output
  log "Running UI tests..."
  yarn test:frontend:ui --reporter=json --outputFile="$UI_TEST_RESULTS_FILE" || true
  
  log "Running Blog tests..."
  yarn test:frontend:blog --reporter=json --outputFile="$BLOG_TEST_RESULTS_FILE" || true
  
  log "Running Bibliography tests..."
  yarn test:frontend:bibliography --reporter=json --outputFile="$BIBLIOGRAPHY_TEST_RESULTS_FILE" || true
  
  if [ $TEST_EXIT_CODE -ne 0 ]; then
    log "❌ Frontend tests failed with exit code $TEST_EXIT_CODE. Aborting deployment."
    return 1
  else
...
# Clean old test results function
clean_old_test_results() {
  local test_dir=$1
  local keep_count=5
  
  log "Cleaning old test results in $test_dir, keeping newest $keep_count"
  ...
  if [ "$file_count" -gt "$keep_count" ]; then
    ...
    find "$test_dir" -type f | sort -r | tail -n +$((keep_count+1)) | xargs -r rm
    log "Removed $(($file_count-$keep_count)) old test results"
  else
    log "No old test results to remove"
  fi
}
...
# Step 2: Run backend tests and verify results
if ! run_backend_tests_and_verify; then
  exit 1
fi

# Step 3: Run frontend tests and verify results
if ! run_frontend_tests_and_verify; then
  exit 1
fi

# Step 4: Apply database migrations if requested
if [ "$DB_MODE" = "y" ]; then
