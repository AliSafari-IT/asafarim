    ...
    try {
      ...
      var updatedData =
        blogPosts && blogPosts.length > 0
          ? { ...dataWithoutEmptyFields, blogPosts }
          : dataWithoutEmptyFields;

      // Remove relatedPosts if it is an empty array
      updatedData =
        model === "topics" && relatedPosts && relatedPosts.length > 0
          ? { ...updatedData, relatedPosts }
          : updatedData;
    ...
    <FluentProvider theme={currentTheme}>
      <div className={classes.formContainer}>
        ...
        >
          {" "}
          {typedData ? (
            Object.entries(typedData as { [key: string]: any })?.map(
              ([key, value]) => (
                <div key={key}>
