...
const MarkdownPage: React.FC<{ data: IMenuItem, title?: string, description?: string, baseUrl?: string }> = ({ data, title, description, baseUrl }) => {
  ...
  const renderDirectoryContent = () => {
    ...
    function getLastChangedAt(createdAt: Date | undefined, updatedAt: Date | undefined): React.ReactNode {
      ...
      if (createdAt && updatedAt) {
        return formatDate(new Date(Math.max(createdAt.getTime(), updatedAt.getTime())));
      } else if (createdAt) {
          ...
          <table className="w-full prose text-left border-collapse">
            <thead>
              <tr className="bg-secondary text-primary">
                ...
                <th className="p-4">Last Modified
                  <SortArray sortOrder={sortOrder} onSortChange={handleSortChange} key={'last-modified-col'} />
                </th>
  ...
  const renderMarkdownContent = () => (
    <div className="prose dark:prose-invert max-w-none" id="markdown-container">
      <DisplayMd id={currentMdFile?.id} markdownContent={currentMdFile?.content || ''} key={currentMdFile?.id} />
    </div>
  );
